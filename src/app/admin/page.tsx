...
export default function AdminPage() {
  ...
  async function suggestCode(categoryId: string) {
    ...
    try {
      const response = await api(`/api/admin/products/next-code?categoryId=${categoryId}`);
      const data = await response.json();
      if (response.ok) setForm((current) => ({ ...current, code: data.code }));
    } catch { setMessage("Não foi possível sugerir o SKU."); }
  }
